...
public class DoubleProbeHashMap<K, V> extends ProbeHashMap<K, V> {
  ...
  /**
    ...
    protected void resize(int newCap) {
    	...
    	this.q = selectSecondaryHashPrime(newCap);
    	super.resize(newCap);    	
    }
    ...
    protected int findSlot(int h1, K k) {
    	...
    	int h2 = secondaryHashValue(k);                            // index while scanning table
    	int j = Math.abs(h1);
        int proby = 0;
        ...
        do {
            ...
            j = (j + proby * secondaryHashValue(k)) % this.capacity;
    ...
    private int secondaryHashValue(K key) {
    	...
    	return this.q - Math.abs(key.hashCode()) % this.q;
    }
