...
public class PatientTriage {
	...
	public Patient remove(Time currentTime)
		...
		if (timeCompare.compare(currentTime, timeHeap.peek().getArrivalTime()) >= 0) {
			if (timeCompare.compare(timeHeap.peek().getArrivalTime().elapsed(currentTime), maxWait) > 0) {
				Patient remPat = timeHeap.poll();
				priorityHeap.remove(remPat.getPriorityPos());
				return remPat;
			}
		}
		Patient remPat = priorityHeap.poll();
		timeHeap.remove(remPat.getPriorityPos());
		return remPat;
	}
